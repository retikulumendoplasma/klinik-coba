            ...
            @forelse ($sudahTindakan as $rekamMedis)
                <tr data-url="/detailTindakanPasien/{{ $rekamMedis->id_rekam_medis }}" style="cursor: pointer;">
                    ...
                    <td>
                        ...
                        <form action="{{ route('tindakan.destroy', $rekamMedis->tindakan->id_tindakan) }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
                            <button class="badge bg-danger border-0" onclick="return confirm('Apakah kamu yakin ingin menghapus data tindakan ini?')">
