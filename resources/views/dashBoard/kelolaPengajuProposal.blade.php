                ...
                @foreach ($pengajuProposal as $proposal)
                    @if ($proposal->status_pengajuan == 'Pending')
                        <tr>
                            ...
                            <td>
                                <a href="/viewProposal/{{ $proposal->id }}" class="badge bg-info">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-eye-fill" viewBox="0 0 16 16">
                                        <path d="M10.5 8a2.5 2.5 0 1 1-5 0 2.5 2.5 0 0 1 5 0"/>
                                        <path d="M0 8s3-5.5 8-5.5S16 8 16 8s-3 5.5-8 5.5S0 8 0 8m8 3.5a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7"/>
                                    </svg>
                                </a>
                                <form action="/terimaPengajuProposal/{{ $proposal->id }}" method="POST" class="d-inline">
                                    @csrf
                                    <button type="submit" class="badge bg-success border-0" onclick="return confirm('Proposal diterima')">
                                ...
                                <!-- Form untuk reject -->
                                <form id="rejectForm{{ $proposal->id }}" action="/kelolaPengajuProposal/{{ $proposal->id }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
                                    <div id="rejectInputContainer{{ $proposal->id }}" class="mb-3" style="display: none;">
                                        <label for="alasan_ditolak" class="form-label">Pesan Penolakan</label>
                                        <textarea class="form-control" id="alasan_ditolak" name="alasan_ditolak" rows="3" required></textarea>
                                        <button type="submit" class="btn btn-danger">Tolak</button>
                                        <button type="button" class="btn btn-secondary" onclick="hideRejectInput('{{ $proposal->id }}')">Batal</button>
                                    </div>
                                    <button type="button" class="badge bg-danger border-0" onclick="showRejectInput('{{ $proposal->id }}')">
